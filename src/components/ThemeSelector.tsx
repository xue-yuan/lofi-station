...
const ThemeSelector: Component<ThemeSelectorProps> = (props) => {
  return (
    ...
    >
      <div class="bg-black/80 backdrop-blur-xl border border-white/10 rounded-2xl overflow-hidden w-[min(300px,calc(100vw-2rem))] shadow-2xl transition-all duration-300">
          ...
          <For each={THEMES}>
            {(theme) => (
              <button
                ...
                aria-pressed={props.activeTheme === theme.id}
              >
                <div
                ...
                />
                <span
                  class={`text-[10px] font-medium tracking-wide ${props.activeTheme === theme.id ? "text-white" : "text-white/60"}`}
                >
                  {theme.name}
                </span>
              </button>
