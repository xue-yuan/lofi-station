...
const Sidebar: Component = () => {
  ...
  return (
    <>
      <button
        class={`fixed left-0 top-1/2 -translate-y-1/2 z-[60] btn btn-sm btn-ghost bg-black/40 backdrop-blur-md border-r border-y border-white/10 rounded-r-lg rounded-l-none text-white/50 hover:text-white hover:pl-4 transition-all duration-300 ${isOpen() ? "translate-x-[min(320px,85vw)]" : "translate-x-0"}`}
        onClick={() => setIsOpen(!isOpen())}
        title="View Changelog"
        aria-label={isOpen() ? "Hide changelog" : "Show changelog"}
      ...
      <div
        aria-hidden={!isOpen()}
        class={`fixed left-0 top-0 bottom-0 w-[min(320px,85vw)] bg-black/80 backdrop-blur-xl border-r border-white/10 z-[60] transition-transform duration-300 transform ${isOpen() ? "translate-x-0" : "-translate-x-full"}`}
      >
        <div class="flex flex-col h-full overflow-hidden">
          ...
          <div class="flex-1 overflow-y-auto custom-scrollbar p-6">
            <div class="flex flex-col gap-8 relative">
              <div class="absolute left-[7px] top-2 bottom-0 w-[1px] bg-white/10" />
              <For each={logs}>
                {(log, index) => (
                  <div class="relative pl-6">
                    ...
                    <div class="flex flex-col gap-2">
                      <div class="flex items-baseline justify-between">
                        <span
                          class={`text-sm font-bold ${index() === 0 ? "text-primary" : "text-white"}`}
                        >
                          v{log.version}
                        </span>
          ...
          <div class="p-4 border-t border-white/10 shrink-0 bg-white/5">
            <p class="text-[10px] text-center text-white/30">Lofi Radio © 2025</p>
          </div>
        </div>
      </div>
      <div
