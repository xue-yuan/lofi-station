      ...
      <div class="w-[min(300px,calc(100vw-2rem))] h-[min(400px,60vh)] bg-black/80 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl flex flex-col overflow-hidden">
        <div class="p-4 border-b border-white/10 flex items-center justify-between shrink-0 bg-white/5">
          ...
          >
            <div class="flex items-center gap-2">
              <button
                class="btn btn-xs btn-ghost btn-circle text-white/60 hover:text-white"
                onClick={handleBack}
              ...
              <h3 class="font-bold text-white tracking-widest uppercase text-xs truncate max-w-[150px]">
          ...
          <button
            ...
            aria-label="Close stations menu"
          >
            ✕
          </button>
        </div>
                    ...
                    <div class="relative w-16 h-10 rounded overflow-hidden shrink-0">
                      ...
                      {playerState.currentChannelId === channel.id && (
                        <div class="absolute inset-0 bg-black/50 flex items-center justify-center">
                          <span class="loading loading-bars loading-xs text-primary" />
                        </div>
                      )}
                    </div>
                    <div class="flex-1 min-w-0">
                      <div
                        class={`text-xs font-bold truncate ${playerState.currentChannelId === channel.id ? "text-primary" : "text-white"}`}
                      >
                        {channel.title}
                      </div>
                      <div class="text-[10px] text-white/50 truncate uppercase tracking-wider">
                        {channel.author}
                      </div>
                    </div>
                  </button>
